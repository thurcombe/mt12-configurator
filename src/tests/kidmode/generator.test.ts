...
import { DEFAULTS } from '../../components/kidmode/kidDefaults.ts';
...
describe('kidGenerator', () => {
  ...
  it('isKidModeActive returns true after applyKidMode', () => {
    const model = loadModel('model00.yml');
    const result = applyKidMode(model, DEFAULTS.crawler.slow, 'FL10');
  ...
  describe('FM1 structure', () => {
    ...
    it('FM1 has fadeIn and fadeOut of 2', () => {
      const model = loadModel('model00.yml');
      const result = applyKidMode(model, DEFAULTS.sport.medium, 'SA0');
      const fm1 = result.flightModeData['1'];
  ...
  describe('expo lines', () => {
    it('adds KID-TH expo line with FM0 excluded', () => {
      const model = loadModel('model00.yml');
      const params = DEFAULTS.crawler.slow;
    ...
    it('adds KID-ST expo line with FM0 excluded', () => {
      const model = loadModel('model00.yml');
      const params = DEFAULTS.rally.medium;
      const result = applyKidMode(model, params, 'FL10');
  ...
  describe('mix lines', () => {
    it('adds KID-SP mix with correct speedUp/speedDown on throttle channel', () => {
      const model = loadModel('model00.yml');
      const params = DEFAULTS.highspeed.slow;
      const result = applyKidMode(model, params, 'FL10');
  ...
  describe('removeKidMode', () => {
    it('removes FM1 from flightModeData', () => {
      const model = loadModel('model00.yml');
      const withKid = applyKidMode(model, DEFAULTS.crawler.slow, 'FL10');
  ...
  it('different vehicle/speed combos produce different throttle rates', () => {
    const model = loadModel('model00.yml');
    const slow = applyKidMode(model, DEFAULTS.crawler.slow, 'FL10');
    const fast = applyKidMode(model, DEFAULTS.crawler.fast, 'FL10');
    const slowRate = slow.expoData.find((l) => l.name === 'KID-TH')!.weight;
    const fastRate = fast.expoData.find((l) => l.name === 'KID-TH')!.weight;
